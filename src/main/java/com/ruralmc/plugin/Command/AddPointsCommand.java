...
public class AddPointsCommand implements CommandExecutor {
    ...
    public AddPointsCommand(RuralMC plugin) {
        this.plugin = plugin;
    }
        ...
        if (cmd.getName().equalsIgnoreCase("addpoints")) {
            if (!(length < 2 || length > 2)) {
                if (src.hasPermission(Permissions.GROUP_STAFF)) {
                    ...
                    src.sendMessage(Messages.CHAT_PREFIX + "Points added to user.");
                    player.sendMessage(Messages.CHAT_PREFIX + ChatColor.GOLD + args[1] + ChatColor.GREEN + " points have been added to your account.");
                }
            } else {
